...
class TestClassification(unittest.TestCase):
    ...
    def test_load_stats_file(self):
        result = cpsign.load_calib_stats(get_resource('cpsign_clf_stats.csv'), sep='\t')
    ...
    def test_plot_single_calib_line(self):
        (signs,errs,errs_sd,labels) = cpsign.load_calib_stats(get_resource('cpsign_clf_stats.csv'), sep='\t')
    ...
    def test_load_stats_label_eff(self):
        (signs,single,multi,empty, _,_,_) = cpsign.load_clf_efficiency_stats(get_resource('cpsign_clf_stats.csv'), sep='\t')
        # print(empty)
        # Explicitly turn of reading of SD values
        (signs,single,multi,empty) = cpsign.load_clf_efficiency_stats(get_resource('cpsign_clf_stats.csv'), sep='\t', prop_e_sd_regex=None, prop_m_sd_regex=None, prop_s_sd_regex=None)
        # print(empty)
    ...
    def test_load_preds(self):
        (ys, pvals, labels) = cpsign.load_clf_predictions(get_resource('cpsign_clf_predictions.csv'),'target',';')
...
class TestRegression(unittest.TestCase):
    ...
    def test_load_reg_calib(self):
        (signs,errs,errs_sd,labels) = cpsign.load_calib_stats(get_resource('cpsign_reg_stats.csv'), sep='\t')
    ...
    def test_load_reg_eff(self):
        (sign_vals, median_widths, mean_widths, median_widths_sd, mean_widths_sd) = cpsign.load_reg_efficiency_stats(get_resource('cpsign_reg_stats.csv'), sep='\t')
